...
const AdminArea: React.FC<AdminAreaProps> = ({ songs, setSongs, settings, setSettings, onClose }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalDownloads,
      totalLikes,
      // Simulando dados temporais baseados nos totais
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-white/5 p-6 flex flex-col md:flex-row items-center justify-between gap-6 sticky top-0 z-50">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex bg-black/40 p-1 rounded-2xl">
            {[
      ...
      <main className="flex-1 p-4 md:p-10 max-w-7xl mx-auto w-full pb-32">
        {activeTab === 'songs' ? (
          <div className="space-y-12 animate-in fade-in duration-500">
            {/* FORMULÁRIO DE UPLOAD COM MÉTRICAS MANUAIS */}
                      ...
                      {songs.map(song => (
                        <tr key={song.id} className="hover:bg-white/[0.02] transition-colors group">
                          ...
                          <td className="p-6 text-center">
                            <input 
                              type="number" 
                              className="w-20 bg-black/40 border border-white/5 p-2 rounded-lg text-center font-mono text-xs text-amber-500" 
                              value={song.plays} 
                              onChange={(e) => updateMetric(song.id, 'plays', parseInt(e.target.value) || 0)}
                            />
                          </td>
                          <td className="p-6 text-center">
                            <input 
                              type="number" 
                              className="w-20 bg-black/40 border border-white/5 p-2 rounded-lg text-center font-mono text-xs text-blue-500" 
                              value={song.downloads} 
                              onChange={(e) => updateMetric(song.id, 'downloads', parseInt(e.target.value) || 0)}
                            />
                          </td>
                          <td className="p-6 text-center">
                            <input 
                              type="number" 
                              className="w-20 bg-black/40 border border-white/5 p-2 rounded-lg text-center font-mono text-xs text-red-500" 
                              value={song.likes} 
                              onChange={(e) => updateMetric(song.id, 'likes', parseInt(e.target.value) || 0)}
                            />
                          </td>
                          <td className="p-6 text-right">
                            <div className="flex justify-end gap-3">
                              <button onClick={() => handleEditClick(song)} className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all">
                                <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} /></svg>
                              </button>
                              <button onClick={() => setSongs(prev => prev.filter(s => s.id !== song.id))} className="w-9 h-9 rounded-lg bg-red-900/20 flex items-center justify-center hover:bg-red-500 group/del transition-all">
                                <svg className="w-4 h-4 text-red-500 group-hover/del:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} /></svg>
                              </button>
                            </div>
                          </td>
                        </tr>
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-3xl font-black uppercase tracking-tighter text-white">Análise de Ranking</h3>
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Comparativo direto de engajamento por faixa</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              {[...songs].sort((a,b) => (b.plays + b.downloads + b.likes) - (a.plays + a.downloads + a.likes)).map((song, idx) => {
                ...
                const max = Math.max(...songs.map(s => s.plays + s.downloads + s.likes)) || 1;
                const percent = (total / max) * 100;

                return (
                  <div key={song.id} className="bg-slate-900/50 border border-white/5 p-6 rounded-[2rem] flex flex-col md:flex-row items-center gap-8 group">
                    <div className="w-12 text-center">
                      <span className={`text-4xl font-black italic ${idx === 0 ? 'text-amber-500' : 'text-slate-700'}`}>#{idx + 1}</span>
                    </div>
                    <img src={song.coverUrl} className="w-24 h-24 rounded-[1.5rem] object-cover shadow-2xl" alt="" />
                    <div className="flex-1 w-full space-y-4">
                      <div className="flex justify-between items-end">
                        <div>
                          <h4 className="text-xl font-black text-white uppercase tracking-tight">{song.title}</h4>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Pontos Totais</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-2">
                           ...
                           <div className="h-1.5 bg-black/40 rounded-full overflow-hidden">
                              <div className="h-full bg-amber-500 transition-all duration-1000" style={{ width: `${(song.plays / (song.plays + song.downloads + song.likes || 1)) * 100}%` }} />
                           </div>
                        </div>
                        <div className="space-y-2">
                           ...
                           <div className="h-1.5 bg-black/40 rounded-full overflow-hidden">
                              <div className="h-full bg-blue-500 transition-all duration-1000" style={{ width: `${(song.downloads / (song.plays + song.downloads + song.likes || 1)) * 100}%` }} />
                           </div>
                        </div>
                        <div className="space-y-2">
                           <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-red-500">
                              <span>Likes</span>
                              <span>{song.likes}</span>
                           </div>
                           <div className="h-1.5 bg-black/40 rounded-full overflow-hidden">
                              <div className="h-full bg-red-500 transition-all duration-1000" style={{ width: `${(song.likes / (song.plays + song.downloads + song.likes || 1)) * 100}%` }} />
                           </div>
                        </div>
                      </div>
                    </div>
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
                   <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                      <svg className="w-24 h-24 text-amber-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>
                   </div>
                ...
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden group" style={{ borderColor: `${settings.accentColor}20` }}>
                   <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                      <svg className="w-24 h-24" style={{ color: settings.accentColor }} fill="currentColor" viewBox="0 0 24 24"><path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1s-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>
                   </div>
                ...
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
                   <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                      <svg className="w-24 h-24 text-red-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                   </div>
             ...
             {/* PERFORMANCE INDIVIDUAL POR MÚSICA */}
             <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] shadow-2xl overflow-hidden">
                <div className="p-8 border-b border-white/5">
                   <h4 className="font-black uppercase tracking-widest text-xs text-white">Auditoria de Desempenho por Faixa</h4>
                </div>
                   ...
                   {songs.map(song => (
                     <div key={song.id} className="bg-black/40 p-6 rounded-3xl border border-white/5 hover:border-white/20 transition-all group">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center border-t border-white/5 pt-4">
                           <div>
                              <p className="text-lg font-black text-amber-500 font-mono">{song.plays}</p>
                              <p className="text-[7px] font-black text-slate-600 uppercase tracking-widest">Plays</p>
                           </div>
                           <div className="border-x border-white/5">
                              <p className="text-lg font-black text-blue-500 font-mono">{song.downloads}</p>
                              <p className="text-[7px] font-black text-slate-600 uppercase tracking-widest">Dwns</p>
                           </div>
                           <div>
                              <p className="text-lg font-black text-red-500 font-mono">{song.likes}</p>
                              <p className="text-[7px] font-black text-slate-600 uppercase tracking-widest">Likes</p>
                           </div>
                        </div>
                     </div>
                   ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-in slide-in-from-bottom-4 duration-500">
            <section className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Nome da Plataforma</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Título do Banner (Hero)</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Subtítulo do Banner</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Rodapé Personalizado</label>
            ...
            <section className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Cor de Destaque</label>
                    <div className="flex gap-4 items-center bg-black/40 p-3 rounded-2xl border border-white/5">
                       <input type="color" value={settings.accentColor} onChange={e => setSettings({...settings, accentColor: e.target.value})} className="w-12 h-12 rounded-xl bg-transparent border-none cursor-pointer" />
                       <span className="text-xs font-mono font-bold uppercase">{settings.accentColor}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Cor de Fundo</label>
                    <div className="flex gap-4 items-center bg-black/40 p-3 rounded-2xl border border-white/5">
                       <input type="color" value={settings.backgroundColor} onChange={e => setSettings({...settings, backgroundColor: e.target.value})} className="w-12 h-12 rounded-xl bg-transparent border-none cursor-pointer" />
                       <span className="text-xs font-mono font-bold uppercase">{settings.backgroundColor}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div 
                    ...
                    className="w-full h-56 bg-black border-2 border-dashed border-white/10 rounded-[2rem] overflow-hidden relative group cursor-pointer"
                  >
                    ...
                    <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                       <svg className="w-10 h-10 text-white mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                       <span className="text-[10px] font-black uppercase tracking-widest">Alterar Imagem da Galeria</span>
                    </div>
                  </div>
